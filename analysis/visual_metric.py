...
# Clean for Backdoor part
clean_true_labels_bd_attack = []
clean_pred_labels_bd_attack = []
clean_ori_labels_bd_attack = []

clean_true_labels_bd_defense = []
clean_pred_labels_bd_defense = []
clean_ori_labels_bd_defense = []

with temporary_all_clean(visual_dataset_bd):
    for i, (inputs, labels, *other_info) in enumerate(data_loader_bd):
        inputs, labels = inputs.to(args.device), labels.to(args.device)
        
        if torch.sum(other_info[1]==1)>0:
            # warning message
            warnings.warn("There are some bd samples in backdoor dataset detected by the poison indicators. Please Check you dataset.")

        # attack part
        outputs = model_attack(inputs)
        prediction = torch.argmax(outputs[:], dim=1)
        clean_true_labels_bd_attack.append(labels.detach().cpu().numpy())
        clean_pred_labels_bd_attack.append(prediction.detach().cpu().numpy())
        clean_ori_labels_bd_attack.append(other_info[2].detach().cpu().numpy())
        
        # defense part
        if model_defense is not None:
            outputs = model_defense(inputs)
            prediction = torch.argmax(outputs[:], dim=1)
            clean_true_labels_bd_defense.append(labels.detach().cpu().numpy())
            clean_pred_labels_bd_defense.append(prediction.detach().cpu().numpy())
            clean_ori_labels_bd_defense.append(other_info[2].detach().cpu().numpy())

    clean_true_labels_bd_attack = np.concatenate(clean_true_labels_bd_attack)
    clean_pred_labels_bd_attack = np.concatenate(clean_pred_labels_bd_attack)
    clean_ori_labels_bd_attack = np.concatenate(clean_ori_labels_bd_attack)

    if model_defense is not None:
        clean_true_labels_bd_defense = np.concatenate(clean_true_labels_bd_defense)
        clean_pred_labels_bd_defense = np.concatenate(clean_pred_labels_bd_defense)
        clean_ori_labels_bd_defense = np.concatenate(clean_ori_labels_bd_defense)
...
# calibration  attack success rate
casr_attack = np.mean((pred_labels_bd_attack==true_labels_bd_attack) & (clean_true_labels_bd_attack==clean_pred_labels_bd_attack))

metric_dic['casr_attack'] = casr_attack


if model_defense is not None:
    casr_defense = np.mean((pred_labels_bd_defense==true_labels_bd_defense) & (clean_true_labels_bd_defense==clean_pred_labels_bd_defense))
    metric_dic['casr_defense'] = casr_defense
