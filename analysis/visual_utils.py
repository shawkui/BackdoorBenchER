...
def plot_embedding(  
    tsne_result, label, title, xlabel="tsne_x", ylabel="tsne_y", custom_palette=None, size=(10, 10), mark_size = 40, alpha = 0.6,style_indicator = None
):
    ...
    if style_indicator is not None:
        sns.scatterplot(
            x="feature_x",
            y="feature_y",
            hue="label",
...
def tsne_fig(
    ...
    fig = plot_embedding(tsne_result, label, title, xlabel,
                         ylabel, custom_palette, size, mark_size, alpha, style_indicator)
    return fig
